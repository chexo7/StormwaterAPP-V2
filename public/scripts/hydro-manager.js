// --- LOGGING ---
        ...
        function openPopup(p,pos){
            ...
            d.className='iw-container p-4 min-w-[260px] max-w-[320px]';

            const drainageSelect = () => {
                const current = props.dischargePoint || '';
                const opts = Array.from({length:5},(_,i)=>`<option value="DP-${i+1}" ${current===`DP-${i+1}`?'selected':''}>DP-${i+1}</option>`).join('');
                return `
                    <div class="text-xs font-bold text-indigo-600 border-b mb-2">Drainage Area</div>
                    <label class="text-[11px] text-slate-500 font-semibold flex flex-col gap-1">
                        <span>Discharge Point</span>
                        <select id="popup-dp" class="border rounded px-2 py-1 text-xs">${opts}</select>
                    </label>
            ...
            const subSelect = () => {
                const drainagePolys = appState.layers.drainage.polygons;
                const hasDrainage = drainagePolys.length>0 && drainagePolys.every(p=>p.feature?.properties?.dischargePoint);
                const current = props.drainageArea || '';
                const opts = Array.from({length:10},(_,i)=>`<option value="Drainage Area - ${i+1}" ${current===`Drainage Area - ${i+1}`?'selected':''}>Drainage Area - ${i+1}</option>`).join('');
                return `
                    <div class="text-xs font-bold text-indigo-600 border-b mb-2">Sub Area</div>
                    ${hasDrainage ? '' : '<div class="text-[11px] text-amber-600 bg-amber-50 border border-amber-100 rounded p-2 mb-2">Complete primero los puntos de descarga de las Drainage Areas para habilitar esta selección.</div>'}
                    <label class="text-[11px] text-slate-500 font-semibold flex flex-col gap-1">
                        <span>Drainage Area</span>
                        <select id="popup-da" class="border rounded px-2 py-1 text-xs" ${hasDrainage?'':'disabled'}>
                            <option value="">Seleccione...</option>
                            ${opts}
                        </select>
                    </label>
                    <div class="flex justify-end gap-2 mt-3">
                        <button id="popup-save" class="px-3 py-1 text-xs bg-indigo-600 text-white rounded shadow hover:bg-indigo-700" ${hasDrainage?'':'disabled'}>Guardar</button>
                        <button id="popup-close" class="px-3 py-1 text-xs border rounded text-slate-600 hover:bg-slate-50">Cerrar</button>
                    </div>`;
            };

            if(p.type==='drainage') d.innerHTML = drainageSelect();
            else if(p.type==='sub') d.innerHTML = subSelect();
            else return;
            ...
            if(saveBtn && p.feature){
                saveBtn.addEventListener('click',()=>{
                    if(p.type==='drainage'){
                        const dp = d.querySelector('#popup-dp')?.value || '';
                        p.feature.properties.dischargePoint = dp;
                        sysLog.add('GIS', `Drainage DP seteado a ${esc(dp)}`, 'success');
                    }
                    if(p.type==='sub'){
                        const da = d.querySelector('#popup-da')?.value || '';
                        p.feature.properties.drainageArea = da;
                        sysLog.add('GIS', `Sub Area asociada a ${esc(da)}`, 'success');
                    }
                    infoWindow.close();
                });
            }
