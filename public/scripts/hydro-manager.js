// --- LOGGING ---
        ...
        function openPopup(p,pos){
            ...
            const dischargeOptions = ['DP-1','DP-2','DP-3','DP-4','DP-5'];
            const drainageAreaOptions = Array.from({length:10},(_,i)=>`Drainage Area - ${i+1}`);
            ...
            if(p.type==='drainage'){
                ...
                d.innerHTML=`
                    ...
                    <label class="text-[11px] text-slate-500 font-semibold flex flex-col gap-1 mb-3">
                        ...
                        <select id="popup-discharge" class="border rounded px-2 py-1 text-xs">
                            <option value="">Seleccione un punto</option>
                            ${dischargeOptions.map(o=>`<option value="${o}" ${o===current?'selected':''}>${o}</option>`).join('')}
                        </select>
                    </label>
                ...
                const drainageFeatures = appState.layers.drainage.polygons.map(x=>x.feature).filter(Boolean);
                const drainageReady = drainageFeatures.length>0 && drainageFeatures.every(f=>f.properties?.dischargePoint);

                if(!drainageReady){
                    d.innerHTML=`
                        <div class="text-xs font-bold text-indigo-600 border-b mb-2">Sub Area</div>
                        <div class="text-xs text-red-600">Asigna primero un Discharge Point a todas las Drainage Areas para habilitar esta selección.</div>
                        <div class="flex justify-end gap-2 mt-3">
                            <button id="popup-close" class="px-3 py-1 text-xs border rounded text-slate-600 hover:bg-slate-50">Cerrar</button>
                        </div>
                    `;
                    ...
                    d.innerHTML=`
                        ...
                        <label class="text-[11px] text-slate-500 font-semibold flex flex-col gap-1 mb-3">
                            ...
                            <select id="popup-drainage-area" class="border rounded px-2 py-1 text-xs">
                                <option value="">Seleccione un área</option>
                                ${drainageAreaOptions.map(o=>`<option value="${o}" ${o===current?'selected':''}>${o}</option>`).join('')}
                            </select>
                        </label>
                        <div class="flex justify-end gap-2 mt-3">
                            <button id="popup-save" class="px-3 py-1 text-xs bg-indigo-600 text-white rounded shadow hover:bg-indigo-700">Guardar</button>
                ...
                d.innerHTML=`
                    <div class="text-xs font-bold text-indigo-600 border-b mb-2">Información</div>
                    <div class="text-xs text-slate-600">No hay acciones disponibles para este polígono.</div>
                    <div class="flex justify-end gap-2 mt-3">
                        <button id="popup-close" class="px-3 py-1 text-xs border rounded text-slate-600 hover:bg-slate-50">Cerrar</button>
                    </div>
                `;
            ...
            if(saveBtn){
                saveBtn.addEventListener('click',()=>{
                    if(p.type==='drainage'){
                        const sel=d.querySelector('#popup-discharge');
                        if(sel){
                            const val=sel.value;
                            if(!val) { sel.classList.add('border-red-500'); return; }
                            p.feature.properties.dischargePoint = val;
                            sysLog.add('GIS', `Discharge Point asignado: ${val}`, 'success');
                        }
                    }
                    if(p.type==='sub'){
                        const sel=d.querySelector('#popup-drainage-area');
                        if(sel){
                            const val=sel.value;
                            if(!val) { sel.classList.add('border-red-500'); return; }
                            p.feature.properties.drainageArea = val;
                            sysLog.add('GIS', `Sub Area asignada a ${val}`, 'success');
                        }
                    }
                    infoWindow.close();
                });
